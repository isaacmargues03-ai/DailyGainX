 ...
 * Implementação robusta com as novas credenciais hardcoded para diagnóstico.
 */
export async function generatePixQrCode(options: GeneratePixOptions): Promise<QrCodeResponse> {
    ...
    // CREDENCIAIS ATUALIZADAS (Chave nova fornecida pelo usuário)
    const clientId = 'Aducmartins_4621537998005562'.trim();
    const clientSecret = '6e7d949e6f87eaad1674807375749a9f21f6cf73769cfed1409bdfc0f7474fcd6e7d949e6f87eaad1674807375749a9f21f6cf73769cfed1409bdfc0f7474fcd'.trim();
    ...
    try {
        ...
        console.log('Solicitando token PixUp com nova chave...');
        ...
        if (!tokenResponse.ok) {
            ...
            console.error('ERRO COMPLETO DA PIXUP (AUTENTICAÇÃO):', errorText);
            throw new Error(`Erro de autorização (401). Verifique se a chave nova foi ativada corretamente.`);
        }
        ...
        if (!qrResponse.ok) {
            ...
            console.error('ERRO COMPLETO DA PIXUP (GERAÇÃO):', errorBody);
